...
function create_pages($title, $template_name, $index_or_front) {
  $new_page_title = $title;
  $new_page_content = '';
  $new_page_template = 'static-front.php'; //ex. template-custom.php. Leave blank if you don't want a custom page template.
  ...
  if(!isset($page_check->ID)){
    ...
    if (strcmp ($index_or_front, 'index') == 0 ){ # Set index
...
if (isset($_GET['activated']) && is_admin()){
  create_pages('Front Page', false, 'static-front.php', 'front');
  create_pages('Index', true , '', 'index');
}
